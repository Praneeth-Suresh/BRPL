...
| BRPL unit tests | `BRPL_ENFORCEMENT=off PYTHONPATH=.beryl python3 -m unittest discover .beryl/brpl/tests -p 'brpl_v*_test.py'` | available | Deterministic v2 evaluator and prospective v3 compiler tests; wired into aggregate gate only in source/self-test mode |
| BRPL policy gate | `BRPL_BASE_REF=<ref> ./.beryl/scripts/check-brpl.sh` | available | Default-inactive v2/v3 evaluation; explicit v4 requires `BRPL_POLICY_VERSION=v4`, enforcement, and external repository/task policies, catalog, evidence, and launch manifest. |
