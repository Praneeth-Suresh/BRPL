...
| BRPL Policy Gate | Versioned repository/task policy schema, strict policy parser, segment-aware glob engine, Git diff/final-tree analyzers, deterministic policy evaluator, stable reports, trusted check adapter registry format, and CLI wrapper under `.beryl/brpl/` | Host functional correctness, natural-language policy rendering, untrusted command execution from policy files, and external evaluator storage | `.beryl/brpl/__init__.py`, `PYTHONPATH=.beryl python3 -m brpl`, `.beryl/scripts/check-brpl.sh` |
