...
| BRPL Policy Gate | Versioned repository/task policy schema, strict policy parser, segment-aware glob engine, deterministic policy evaluator, stable reports, trusted adapter catalogs, and CLI wrappers under `.beryl/brpl/` | Host functional correctness, natural-language policy rendering, untrusted command execution from policy files, adapter extraction internals, and external evaluator storage | `.beryl/brpl/__init__.py`, `PYTHONPATH=.beryl python3 -m brpl`, explicit `PYTHONPATH=.beryl python3 -m brpl.v4`, `.beryl/scripts/check-brpl.sh` |
...
- `.beryl/scripts/check-brpl.sh` is the BRPL safety-gate adapter. In default mode it runs only when BRPL policy files or BRPL policy variables are configured; inactive installs do not require Python. When evaluation runs, it requires Python >=3.11, an explicit `BRPL_BASE_REF`, and delegates policy evaluation to the `.beryl/brpl` Python library. `BRPL_ENFORCEMENT=enforce` requires explicit evaluator-owned repository policy, task policy, and check registry paths outside the evaluated repo and fails closed on absent, unreadable, repo-local, wrong-kind, or wrong-shape inputs.
- BRPL v4 is an explicit prospective API and uses a host-neutral compiler/verifier over normalized evidence; language, manifest, Git, metric, and command extraction are external trusted adapters, with the Python development adapter exposed only through `.beryl/brpl/v4/adapter_driver.py`, while enforced v4 launch manifests pin and recheck the catalog, adapter bundle, checker, baseline, evaluator, policies, externally controlled evidence/run record, and candidate hash.
