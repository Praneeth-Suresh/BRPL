...
The development adapter is explicitly selected with `python3 -m brpl.v4.adapter_driver --adapter python-evidence-bundle`; it is outside the compiler/verifier boundary, emits complete evidence only within its declared static-Python candidate-file universe, binds supplied fixed check results to the candidate hash, and prints the artifact SHA-256 that an external catalog must pin. Dynamic imports and runtime resolution remain outside that universe. Regenerate a catalog digest whenever the reviewed adapter artifact changes.
